...
function simplent_entry_meta() {
	...
	printf( '<span class="author-info"> by <a href="%1$s">%2$s</a></span>', $author_url, get_the_author_meta( 'display_name', $author_id ) );
    ...
    if( ( $categories_list || $tags_list ) || get_edit_post_link() ) :
        ...
        //if( 'post' === get_post_type() && is_single() ) {
            ...
            if( $categories_list || $tags_list ) {
                    ...
                    if( $categories_list ) {
                        echo '<span class="cat-links">' . '<span class="cat-icon"><i class="fa fa-folder-open"></i></span>' . '<span class="screen-reader-text">' . esc_attr__( 'Categories', 'simplent' ) . '</span>' . $categories_list . '</span>';
                    }

	            if( 'post' === get_post_type() && is_single() ) {
		            if ( $tags_list ) {
			            echo '<span class="tags-links">' . '<span class="tags-icon"><i class="fa fa-hashtag" aria-hidden="true"></i></span>' . '<span class="screen-reader-text">' . esc_attr__( 'Tags', 'simplent' ) . '</span>' . $tags_list . '</span>';
		            }
	            }
	    ...
	    edit_post_link(
		    ...
		    /* translators: %s: Name of current post */
			    __( 'Edit<span class="screen-reader-text"> "%s"', 'simplent' ),
			    get_the_title()
		    ),
